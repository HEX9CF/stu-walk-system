...
public class Dijkstra {
    ...
    public static void getPath(int start, int end) {
        if (start != end) {
            getPath(start, prev[end]); // Recursively trace the path back to the start
        }
        list.add(end + 1); // Add the current node to the path list (1-based index)
    }
    ...
    public static List<Integer> dijkstra(int start, int end) {
        int vertex = 54; // Total number of nodes (vertices) in the graph
        ReadSheet5 readSheet5 = new ReadSheet5(); // Helper class to read the graph data
        dis = readSheet5.read(); // Read the graph data (distance matrix)
